...
if __name__=='__main__':
    ...
    #print(md5(tmp[i], stopAtRound),' <= "',tmp[i].decode('ascii'),'"', sep='')
    '''

    stopAtRound = int( input("When stop?\nAt round # ") )
    size = len(tmp)

    Output = open("Result.txt", "w")
    if Output.mode == 'w':
        Output.write(str(size) + " " + str(stopAtRound) + "\n")
        #Output.write(" ")
        #Output.write(stopAtRound)
        #Output.write("\n")
        for i in range(size):
            #print(md5_to_hex(md5(tmp[i], stopAtRound)),' <= "',tmp[i].decode('ascii'),'"', sep='')

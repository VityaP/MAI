...
std::shared_ptr<TBinaryTreeItem<T>> TBinaryTree<T>::deleteNode(std::shared_ptr<TBinaryTreeItem<T>> root, int32_t side)
{
    ...
    if (side < root->item->Side()) {
        ...
        if (root->left == nullptr && root->right == nullptr) {
            std::shared_ptr<TBinaryTreeItem<T>> temp = nullptr;
            root->parent = nullptr;
            return temp;
        } else if (root->left == nullptr) {
            std::shared_ptr<TBinaryTreeItem<T>> temp = nullptr;
            temp = std::make_shared<TBinaryTreeItem<T>>(root->right->item, nullptr, nullptr, root->parent);
            ...
            std::shared_ptr<TBinaryTreeItem<T>> temp = nullptr;
            temp = std::make_shared<TBinaryTreeItem<T>>(root->left->item, nullptr, nullptr, root->parent);

...
#define _sizeShared(id) ((id) + ((id) / 31) + 10)
// 0   1   2   3   4   ... 29  30  X
...
__global__ void BitonicSortShared(int* nums, int n, int iterShift, int iterToStart){
    ...
    for(int i = idx; i < n; i += offset){
        ...
        for (iter = iterToStart; iter > 0; iter /= 2){
            for (localStep = iter; localStep > 0; localStep /= 2){
...
int main(){
    ...
    for(iteration = 1; iteration < powerBound; iteration *= 2){
        localShift = iterShift;
        for(localStep = iteration; localStep > 0; localStep /= 2){

            if (localStep == SharedSize / 2){
                BitonicSortShared<<<4, ThreadPerBlock>>>(device_nums, powerBound, iterShift + 1, localStep);
                CSC(cudaGetLastError());
                break;
            }
            else{
                BitonicSort<<<64, 256>>>(device_nums, powerBound, iterShift + 1, localStep, localShift);
                CSC(cudaGetLastError());
            }
